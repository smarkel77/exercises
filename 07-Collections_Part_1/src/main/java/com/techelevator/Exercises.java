...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		for (String stringArrayi : stringArray) {
			newArray.add(stringArrayi);
		}
		return newArray;
	}
	...
	public String[] list2Array(List<String> stringList) {
		...
		for (String stringListI : stringList) {
			newArray[counter] = stringListI;
			counter++;
		}
		return newArray;
	}
	...
	public List<String> no4LetterWords(String[] stringArray) {
		List<String> not4 = new ArrayList<String>();
		for (String stringArrayI : stringArray) {
			if (stringArrayI.length() != 4) {
				not4.add(stringArrayI);
			}
		}
		return not4;
	}
	...
	public List<String> reverseList(List<String> stringList) {
		...
		for (String stringListI : stringList) {
			stackList.push(stringListI);
		}
		while (stackList.size() > 0) {
			reverseOrder.add(stackList.pop());
		}
		return reverseOrder;
	}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (int intArrayi : intArray) {
			Double divide = (intArrayi * 1.0) / 2;
			doubleList.add(divide);
		}
		return doubleList;
	}
	...
	public Integer findLargest(List<Integer> integerList) {
		Integer largest = 0;
		for (Integer integerI : integerList) {
			if (integerI > largest) {
				largest = integerI;
			}
		}
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		for (Integer integerI : integerArray) {
			if (integerI % 2 == 1) {
				onlyOdd.add(integerI);
			}
		}
		return onlyOdd;
	}
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for (Integer integerI : integerArray) {
			if (integerI % 3 == 0 && integerI % 5 == 0) {
				newArray.add(multiple3And5);
			} else if (integerI % 3 == 0) {
				newArray.add(multiple3);
			} else if (integerI % 5 == 0) {
				newArray.add(multiple5);
			} else {
				newArray.add(integerI.toString());
			}
		}
		return newArray;
	}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		queueOne.addAll(listOne);
		Queue<Integer> queueTwo = new LinkedList<Integer>();
		queueTwo.addAll(listTwo);

		int maxLength = 0;
		if (queueOne.size() >= queueTwo.size()) {
			maxLength = queueOne.size();
		} else {
			maxLength = queueTwo.size();
		}

		for (int i = 0; i < maxLength; i++) {
			if (!queueOne.isEmpty()) {
				newArray.add(queueOne.poll());
			}
			if (!queueTwo.isEmpty()) {
				newArray.add(queueTwo.poll());
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		List<Integer> oneToTen = new ArrayList<Integer>();
		List<Integer> elevenToTwenty = new ArrayList<Integer>();
		List<Integer> twentyOneToThirty = new ArrayList<Integer>();
		for (Integer seatNumber : seatNumberList) {
			if (seatNumber >= 1 && seatNumber <= 10) {
				oneToTen.add(seatNumber);
			}
			if (seatNumber >= 11 && seatNumber <= 20) {
				elevenToTwenty.add(seatNumber);
			}
			if (seatNumber >= 21 && seatNumber <= 30) {
				twentyOneToThirty.add(seatNumber);
			}
		}
		newList.addAll(oneToTen);
		newList.addAll(elevenToTwenty);
		newList.addAll(twentyOneToThirty);
